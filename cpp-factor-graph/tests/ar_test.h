...
TEST(AutoRegressive, DISABLED_RLS) {
    ...
    noise_W->receive(makeGaussian(0, REAL_varW, GaussianMessage::GAUSSIAN_VARIANCE));
...
TEST(AutoRegressive, NoObservationNoise) {
    ...
    auto varU_equ = new EqualityNode;
    auto varU_prev = new EvidenceNode;
    auto varU_next = new EvidenceNode;
    ...
    nwk.addEdge(varU_prev, varU_equ);
    nwk.addEdge(varU_equ, varU_next);
    nwk.addEdge(varU_equ, var);
